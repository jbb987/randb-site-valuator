...
import {
  ...
  JOB_DOCUMENT_CATEGORY_LABELS,
  MAX_DOCUMENT_BYTES,
  type ConstructionJob,
  type JobDocument,
  type JobDocumentCategory,
} from '../../types';
...
function isImage(d: JobDocument): boolean {
  return d.contentType.startsWith('image/');
}

function isPdf(d: JobDocument): boolean {
...
export default function JobDocumentsSection({ job, perms }: Props) {
  ...
  async function handleOpen(d: JobDocument) {
    try {
  ...
  async function handleDownload(d: JobDocument) {
    try {
      const blob = await downloadBlob(d);
      ...
      a.click();
      // Defer cleanup — Safari kills the download if we revoke synchronously.
      setTimeout(() => {
        document.body.removeChild(a);
        URL.revokeObjectURL(objectUrl);
      }, 2000);
    } catch (err) {
      console.error('[JobDocuments] Download failed:', err);
      try {
        const url = await openUrl(d);
        window.open(url, '_blank', 'noopener');
        setError(
      ...
      {loading ? (
        ...
        <div className="text-center py-10 text-sm text-[#7A756E]">
          ...
          {perms.canUploadDocuments && (
            <>
              {' '}
              Click <span className="font-medium text-[#201F1E]">Upload</span> to add one.
            </>
          )}
        </div>
      ) : (
      ...
      >
        <div className="min-w-0 flex-1">
          ...
          <div className="text-xs text-[#7A756E] mt-0.5 truncate">
            {formatDate(doc.uploadedAt)} · {doc.uploadedByEmail ?? 'Unknown'} ·{' '}
