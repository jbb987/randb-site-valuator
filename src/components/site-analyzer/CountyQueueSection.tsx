    ...
    return (
      <div className="bg-white rounded-2xl border border-[#D8D5D0] p-5 md:p-6 mt-5">
        ...
        <p className="text-xs text-[#7A756E]">
          Set <span className="font-medium">{missing}</span> on this site (edit the site or re-run
          analysis) to see county-level queue data.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#D8D5D0] p-5 md:p-6 mt-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-y-2 gap-x-6 text-xs mb-5">
        <div className="flex justify-between">
          ...
          <span className="font-medium text-[#201F1E]">
            {fmtCount(data.active_count)} · {fmtMW(data.active_mw)}
          </span>
        </div>
        <div className="flex justify-between">
          <span className="text-[#7A756E]">Withdrawn (5y)</span>
          <span className="font-medium text-[#201F1E]">
            {fmtCount(data.withdrawn_count_5y)} · {fmtMW(data.withdrawn_mw_5y)}
          </span>
        </div>
        <div className="flex justify-between">
          <span className="text-[#7A756E]">In service</span>
          <span className="font-medium text-[#201F1E]">
            {fmtCount(data.in_service_count)} · {fmtMW(data.in_service_mw)}
          </span>
        </div>
        {wdRate != null && (
          <div className="flex justify-between">
        ...
        {data.earliest_active_cod && (
          <div className="flex justify-between">
            ...
            <span className="font-medium text-[#201F1E]">
              {fmtCodYear(data.earliest_active_cod)}
            </span>
          </div>
        )}
      </div>
      ...
      {fuelEntries.length > 0 && (
        <div className="mb-5">
          <div className="text-[10px] uppercase tracking-wide text-[#7A756E] mb-2">
            Active queue fuel mix
          </div>
            ...
            {fuelEntries.map(([fuel, share]) => {
              ...
              return (
                <div key={fuel} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="w-16 text-right text-[#7A756E] tabular-nums shrink-0">
                    {fmtMW(mwShare)}
                  </span>
                </div>
              );
          ...
          <div className="space-y-1">
            {voltageEntries.map(([volt, share]) => (
              <div key={volt} className="flex items-center gap-2 text-xs">
                ...
                <div className="flex-1 h-2 bg-stone-100 rounded overflow-hidden">
                  <div
                    className="h-full bg-[#7A756E] rounded"
              ...
              <tbody>
                {data.top_active.slice(0, 10).map((p, i) => (
                  <tr key={i} className="border-b border-[#D8D5D0]/50 last:border-0">
                    <td className="py-1.5 pr-2 text-[#201F1E] truncate max-w-[280px]">
                      {p.name || 'Unnamed'}
                    </td>
                    <td className="py-1.5 px-2 text-right text-[#201F1E] font-medium tabular-nums">
                      {fmtMW(p.mw)}
                    </td>
                    <td className="py-1.5 px-2 text-[#7A756E]">{FUEL_LABEL[p.fuel]}</td>
                    <td className="py-1.5 px-2 text-[#7A756E] tabular-nums">
